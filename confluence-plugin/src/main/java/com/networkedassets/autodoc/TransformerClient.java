...
public class TransformerClient {
	...
	public HttpResponse<String> setCredentials(Settings settings) throws SettingsException {
		try {
			return Unirest.post(url + CREDENTIALS).header("Content-Type", "application/json").body(settings).asString();
		} catch (UnirestException e) {
			throw new SettingsException(e);
		}
	}

	public HttpResponse<String> modifyBranch(int sourceId, String projectKey, String repoSlug, String branchId,
			Branch branch) throws SettingsException {
		try {
			return Unirest.put(url + "/sources/{sourceId}/{projectKey}/{repoSlug}/{branchId}")
	...
	public HttpResponse<String> forceGenerate(String sourceUrl, String projectKey, String repoSlug, String branchId)
		...
		try {
			return Unirest.post(url + EVENT).header("Content-Type", "application/json").body(eventPayload).asString();
		} catch (UnirestException e) {
			throw new SettingsException(e);
		}
	}
	...
	public HttpResponse<String> setSource(Source source) throws SettingsException {
		try {
			return Unirest.post(url + SOURCES).header("Content-Type", "application/json").body(source).asString();
		} catch (UnirestException e) {
			throw new SettingsException(e);
		}
	}

	public HttpResponse<String> changeSource(String id, Source source) throws SettingsException {
		try {
	...
	public HttpResponse<String> removeSource(String id) throws SettingsException {
		try {
			return Unirest.delete(url + SOURCES + "{id}").routeParam("id", id).asString();
		} catch (UnirestException e) {
			throw new SettingsException(e);
		}
	}
