...
public class PlantUML {
  ...
  private String generateAsText(@Nonnull File directory) throws PlantUMLException {
    ...
    try {
      ...
      final CommandLine commandLineArguments = new CommandLineImpl(
          new String[] { "-o", tempFile.getAbsolutePath(), "-b", directory.getAbsolutePath(), "-e",
              "**/package-info.java" });
  ...
  private static void cloneTheRepo(@Nonnull SCM scmClient, @Nonnull String projectKey,
      @Nonnull String repositorySlug, @Nonnull String branchName, @Nonnull File localDirectory)
          throws PlantUMLException {
    try {
  ...
  private static void createDirectoryIfNecessary(@Nonnull File localDirectory)
      throws PlantUMLException {
    Preconditions.checkNotNull(localDirectory);
    if (!(localDirectory.isDirectory() || localDirectory.mkdirs())) {
      throw new PlantUMLException("could not create the directory: " + localDirectory.getPath());
    }
  }
